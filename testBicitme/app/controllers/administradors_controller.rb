class AdministradorsController < ApplicationController
  ...
  def create
    @administrador = Administrador.new()
    @administrador.admin_run = current_user.usuar_run
    @administrador.admin_cant_aceptados = 0
    @administrador.admin_cant_ban_cic = 0
    @administrador.admin_cant_ban_taller = 0
    @administrador.usuario_id = current_user.id
  ...
  private
    ...
    def administrador_params
      params.require(:administrador).permit(:admin_run, :admin_cant_aceptados, :admin_cant_ban_cic, :admin_cant_ban_taller, :usuario_id)
    end
end
